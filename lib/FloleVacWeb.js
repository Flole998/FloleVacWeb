...
class FloleVacWeb {
    constructor() {
        ...
        this.model = new Model(FloleVacWeb.VACUUM_MODEL_PROVIDER());
        this.tokenProvider = FloleVacWeb.NATIVE_TOKEN_PROVIDER;
        ...
        const robotArgs = {
            ...
            cloudKey: FloleVacWeb.CLOUD_KEY_PROVIDER(),
            deviceId: FloleVacWeb.DEVICE_ID_PROVIDER(),
        };

        Logger.info("Starting FloleVacWeb for Vacuum Model " + (this.model ? this.model.name : "UNKNOWN"));
    ...
    async shutdown() {
        Logger.info("FloleVacWeb shutdown in progress...");

        const forceShutdownTimeout = setTimeout(() => {
            Logger.warn("Failed to shutdown floleVacWeb in a timely manner. Using (the) force");
            process.exit(1);
        }, 5000);

        // shuts down floleVacWeb (reverse startup sequence):
        if (this.mqttClient) {
        ...
        Logger.info("FloleVacWeb shutdown done");
        clearTimeout(forceShutdownTimeout);
    }
}

module.exports = FloleVacWeb;
