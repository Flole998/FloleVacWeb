...
class WebServer {
    ...
    initWebSocketServer(server) {
        ...
        this.events.on("floleVacWeb.map", () => {
            ...
            zlib.deflate(JSON.stringify(this.map.parsedData), (err, buf) => {
                //Too many connected clients might cause floleVacWeb to go OOM
...
module.exports = WebServer;
    this.app.get("/api/map/raw/latest", function (req, res) {
        res.status(200).send(self.map.rawData)
    });

    this.app.get("/api/flolevac/setup", function (req, res) {
        self.vacuum.getCurrentStatus(function (err, data) {
            if (err) {
                ...
                res.json({
                    ...
                    did: self.vacuum.deviceId,
                    model: self.vacuum.model
                }); 
            }
        });
    });
