...
const uploadLocation = "/mnt/data/floleVacWeb/uploads";
    ...
    router.get("/get_fw_version", async (req, res) => {
        try {
            ...
            if (extractedOsRelease) {
                ...
                let floleVacWebVersion = "?"; //Could not read ../package.json
                if (packageContent) {
                    floleVacWebVersion = JSON.parse(packageContent).version;
                }
                //return result
                res.json({
